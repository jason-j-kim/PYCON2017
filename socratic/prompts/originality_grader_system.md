...
| **미실행** | 조회 자체를 안 함(키·데이터 없음) | **아무 정보도 아니다.** 근거로 전혀 쓰지 않는다 |
| **조회 실패** | 시도했으나 오류로 끝남(망 장애 등) | **아무 정보도 아니다.** 미실행과 같이 다룬다 |

- 미실행·조회 실패 통로를 두고 "0건", "흔적이 없다", "시도되지 않았다"고 **쓰지
  않는다.** 그 통로에 대해 아는 바가 없으므로 아는 척하지 않는다.
- `reasoning`에는 **어느 통로가 미실행·조회 실패였는지 한 번은 밝힌다.** 예: "의안
  통로는 미실행이라 입법 시도 여부는 확인하지 못했다."
- **미실행·조회 실패 통로가 하나라도 있으면 확신도 `상`을 쓰지 않는다**(최대 `중`).
  덮지 못한 영역이 남아 있기 때문이다.
- `retraction_condition`에 그 통로를 돌렸을 때 선례가 나오면 하향한다는 취지를 적는다.
