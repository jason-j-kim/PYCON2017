...
// ═══════════ 1. 표지 ═══════════
{
  ...
  const tags = ['12문답 · 5단계', '이원 채점', '검증 통로 4개', 'KDI 7,362건', 'OPSI 1,015건'];
  tags.forEach((t, i) => {
...
// ═══════════ 5. 세 한계 (VS) ═══════════
{
  ...
  const rows = [
    ...
    ['선례 확인이 담당자 검색 역량에 좌우', '4개 국가 데이터를 매번 동일하게 조회'],
...
// ═══════════ 6. 해법의 방향 ═══════════
{
  ...
  const cols = [
    ...
    ['02', '데이터 대조', ['예산·연구·입법·해외를 교차 확인', '담당자가 바뀌어도 동일 결과',
  ...
  const axes = [
    ['축 A', '대화 채점', '제안이 얼마나 잘 방어되는가',
      ['12문답에서 드러난 진술을 평가', '체크리스트 30항목 + 종합판단',
       '독창성 35% · 실용성 35% · 수용태도 30%', '산출: 0~10 점수 + 신뢰구간'], C.PRIMARY],
...
// ═══════════ 12. 이원 채점 개요 ═══════════
{
  ...
  const two = [
    ['① 체크리스트 채점', '기계적 · 재현 가능', ['기준마다 이진 항목 10개 (총 30개)',
      '"이 사건이 대화에 있었는가"로만 판정', '충족 개수 = 그대로 0~10점', '채점자에 따라 흔들리지 않음'], C.PRIMARY],
...
// ═══════════ 15. 불일치 = 신뢰구간 ═══════════
{
  const s = contentSlide(3);
  tagline(s, '03. 대화와 채점');
  sectionTitle(s, '축 A ⑥ — 두 채점의 불일치가 곧 신뢰구간');
  ...
  bottomBar(s, '점수 하나가 아니라 "6.0 (범위 5.2–6.8)" 형태로 제시 — 불확실성을 감추지 않습니다');
}
...
// ═══════════ 21. 해외사례 ═══════════
{
  ...
  bottomBar(s, '해외 조회에서 걸린 사례는 A급 근거 — 국가·연도·설계까지 인용할 수 있습니다', 5.02);
}
...
// ═══════════ 22. 근거 등급 ═══════════
{
  ...
  const two = [
    ['A급 — 조회로 확인', C.GREEN, 'F0F9F3',
      ['재정·KDI·의안·해외 조회에서 실제로 나온 것', '사업명·연도·부처·예산·의안명 인용 가능',
       '단, 설계가 실질적으로 같음을 함께 논증해야 감점 근거']],
  ...
  two.forEach((t, i) => {
    ...
    bullets(s, t[3], x + 0.18, 2.62, w - 0.36, 10.5, 0.44);
  });
...
// ═══════════ 25. 운영 비용 ═══════════
{
  ...
  txt(s, '아이디어 1건당 약 2.8만~4.3만 토큰 소요 · 프롬프트 캐싱 적용 · 환율 1,500원 기준',
  ...
  txt(s, '월 1,000건을 표준 모델로 처리할 경우 약 29만~36만 원 — 담당자 1인 검토시간과 비교 가능한 수준',
    { x: 0.5, y: 4.78, w: 9, h: 0.3, fontSize: 11.5, bold: true, color: C.PRIMARY, align: 'center' });
  txt(s, '※ 대화 길이(단문/장문)에 따라 변동하며, 반복되는 지시문은 캐싱으로 절감됩니다',
...
// ═══════════ 26. 한계와 다음 단계 ═══════════
{
  ...
  const two = [
    ['현재의 한계', C.AMBER, 'FFF8E8',
      ['지자체 자체사업·기금사업 포착이 불균등', '재정 자료는 최근 5개 연도만 수록',
       '국내 소스는 2000년대 후반 이후를 주로 덮음', '해외 코퍼스는 OECD 등재 사례에 한정',
       '판정은 최종 결론이 아니라 검토 후보 목록']],
    ['다음 단계', C.PRIMARY, 'EFF4FB',
      ['지자체·기금 데이터 통로 추가', '부처 배정·이관 절차와 연계',
       '판정 이력을 국가 아이디어 코퍼스로 축적', '중복 제안 자동 식별 기능 강화',
       '담당자 피드백을 판정 규칙에 반영']],
  ];
  const w = 4.5, gx = 0.2;
  two.forEach((t, i) => {
    ...
    txt(s, t[0], { x: x + 0.18, y: 2.1, w: w - 0.36, h: 0.34, fontSize: 14, bold: true, color: t[1] });
    s.addShape(pres.shapes.RECTANGLE, { x: x + 0.18, y: 2.5, w: w - 0.36, h: 0.012, fill: { color: t[1] } });
    bullets(s, t[3], x + 0.18, 2.64, w - 0.36, 10.5, 0.39);
  });
