...
def inspect(k):
    ...
    say()
    # 콘솔의 키 목록은 각 키의 앞부분을 보여준다. 이것과 대조하면 '폐기된
    # 키'와 '다른 계정의 키'가 갈린다. 108자 중 20자만 보여도 대조는 되고
    # 키가 새지는 않는다.
    if len(k) >= 24:
        say(f"  콘솔 목록과 대조할 앞부분:  {k[:20]}…")
        say("    console.anthropic.com/settings/keys 의 목록에 이렇게 시작하는")
        say("    키가 있는지 보세요. 없으면 폐기됐거나 다른 계정의 키입니다.")
        say()
...
def call(k, base, model):
    ...
    try:
        ...
        low = (msg or "").lower()
        if e.code == 401:
            say("    ▶ Anthropic 이 이 키를 모른다는 뜻입니다.")
            say()
            if len(k) == 108 and k.startswith("sk-ant-api03-"):
                # 여기까지 왔다는 것은 1단계를 통과했다는 뜻이다. 길이도 형식도
                # 정확하면 붙여넣기 손상은 아니다. 남은 답은 하나뿐이다.
                say("      키의 길이와 형식은 정확합니다. 붙여넣다 망가진 것이")
                say("      아닙니다. 그렇다면 이 키는 Anthropic 쪽에 더 이상")
                say("      존재하지 않습니다. 남은 경우는 둘뿐입니다.")
                say()
                say("      1. 콘솔에서 그 키를 지웠거나 다시 만들었다")
                say("      2. 다른 계정·조직에서 만든 키다")
                say()
                say(f"      가리는 법 — {CONSOLE} 을 열고")
                say(f"      목록에 «{k[:20]}…» 로 시작하는 키가 있는지 보세요.")
                say()
                say("        있다  → 그 계정은 맞습니다. 키를 지우고 새로 만드세요.")
                say("        없다  → 폐기됐거나 다른 계정입니다. 콘솔 왼쪽 위에서")
                say("                조직을 바꿔 가며 찾아보고, 없으면 새로 만드세요.")
                say()
                say("      새 키는 만든 직후 화면에서 [Copy] 로 복사하세요.")
                say("      그 화면을 벗어나면 전체 값을 다시는 볼 수 없습니다.")
            else:
